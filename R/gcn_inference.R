...
#'
#' @param exp A gene expression data frame with genes in row names and samples in column names or a `SummarizedExperiment` object.
#' @param net_type Network type. One of 'signed', 'signed hybrid' or 'unsigned'. Default: 'signed'.
...
#' @param reportPDF Logical indicating whether to save 2 PDF figures with heatmap of eigengene networks and dendrogram with modules, respectively. Default: TRUE.
#'
...
#' @param MEs Module eigengenes. It is the 2nd element of the result list generated by the function \code{exp2gcn}.
...
#'
#' @return A data frame with correlation and correlation p-values for each pair of ME and trait along with a heatmap.
...
#' @param exp A gene expression data frame with genes in row names and samples in column names or a `SummarizedExperiment` object.
#' @param metadata A data frame containing sample names in row names and sample annotation in the first column. Ignored if `exp` is a `SummarizedExperiment` object, since the function will extract colData.
#' @param genes Character vector of genes to be correlated with traits. If not given, all genes in `exp` will be considered.
#' @param alpha Significance level. Default is 0.05.
#' @param min_cor Minimum correlation coefficient. Default is 0.2.
#' @param use_abs Logical indicating whether to filter by correlation using absolute value or not. If TRUE, a \code{min_cor} of say 0.2 would keep all correlations above 0.4 and below -0.4. Default is TRUE.
...
#'
#' Perform enrichment analysis for gene functional annotation given as input.
#'
...
#' @details
#' The enrichment analysis is parallelized with BiocParallel, resulting in a dramatic increase in speed.
#' However, if you try to run this function in a laptop with low memory, you might have problems with memory usage.
