...
This function reads multiple expression tables (.tsv files) in a
directory and combines them into a single gene expression data frame.
...
- mypath:
  ...
  Path to directory containing .tsv files. Files must have the first
  column in common, e.g. "Gene_ID". Rows are gene IDs and columns are
  sample names.

- pattern:

  Pattern contained in each expression file. Default is '.tsv\$', which
  means that all files ending in '.tsv' in the specified directory will
  be considered expression files.

## Value

Data frame with gene IDs as row names and their expression values in
each sample (columns).
