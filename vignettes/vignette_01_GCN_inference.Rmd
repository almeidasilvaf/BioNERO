...
For this tutorial, we will use maize (*Zea mays*) gene expression data normalized in TPM. The data was obtained from @Shin2020 and was filtered for package size issues. For more information on the data set, see `?zma.se`. The data set is stored as a SummarizedExperiment object.[^1]
...
plot_heatmap(final_exp, type="expr", cluster_rows=FALSE)
```
...
Here comes the actual network inference! Now, we can use the power calculated by `SFT_fit()` to infer the GCN. The function `exp2gcn()` infers a GCN and outputs a list of 7 elements, each of which will be used by other functions in the analysis pipeline.
...
By default, `exp2gcn()` returns 2 PDF figures in the user's working directory. You can set `reportPDF = FALSE` if you do not want the figures. The figures display i. a dendrogram of genes and modules; ii. heatmap of pairwise correlations between module eigengenes.
...
MEtrait <- module_trait_cor(exp=final_exp, metadata=metadata, MEs=net$MEs, 
                 cor_method="pearson")
head(MEtrait)
```
...
# Check SFT fit
check <- check_sft(edges_filtered, net_type = "gcn")
```
